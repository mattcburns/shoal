...
func cmdDeployRun(args []string) int {
	...
	installStrategy := fs.String("install-strategy", "", "simulate|image_write|operator_iso|scripted_iso (M4 Flatcar/Ubuntu installer+seed)")
	...
	seedDelivery := fs.String("seed-delivery", "", "none|auto|second_media|config_drive (offline seed; no guest HTTP)")
